...
# LC1 = LC-IMPACT v1.3 (built by the appendix at the end of 15_7). The tidy CF files
# are variant-independent, so they always come from the baseline tree.
## LC impact
...
for (nm in names(E_fdem_sua)) {
  saveRDS(E_fdem_sua[[nm]], file.path(EXT_ROOT, "fd_sua", paste0(nm, ".rds")))
  saveRDS(E_fdem_cbs[[nm]], file.path(EXT_ROOT, "fd_cbs", paste0(nm, ".rds")))
}
