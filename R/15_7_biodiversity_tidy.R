...
# above. Both versions are run side by side in 15_8 (LC = v2, LC1 = v1.3), they do
...
# factors used in 15_5. The certain/all axis has no counterpart elsewhere in FABIO
...
# `type` has to be the FABIO pressure name that 15_8 builds its column names from.
...
# becomes a zero at the na.rm = TRUE aggregation in 15_8, again as for v2.
...
# Table 2), average is the closer of the two labels 15_8 knows.
...
# 15_8 knows -> approach = "Average". Both nutrients take the to-water CF (P to
...
# dry run of what 15_8 does with these objects, so a broken CF set fails here rather
...
if(length(lc13_bad_cat)) {
  stop("realm/.id combinations not in 15_8's impact_categories: ",
       paste(lc13_bad_cat, collapse = ", "), ".")
}

if(anyDuplicated(lc13_check, by = c("iso3c", "colname"))) {
  stop("More than one CF per iso3c and column - dcast in 15_8 would silently aggregate.")
}
